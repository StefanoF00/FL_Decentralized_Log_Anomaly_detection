...
def simulate_network(N, M, T, dt, A, k1, k2, alpha, delta1, initial_states, model, dist=None):
    T_max = np.max(T)
    t_eval = np.linspace(0, T_max, int(T_max / dt))  # Vettore dei tempi di valutazione
...
def plot_convergence_M_states(x_trajectory, time):
    ...
    # Calcola i valori di consenso finali per ogni integratore
    consensus_values = np.mean(x_trajectory[-1, :, :], axis=0)
    
    # Plotta le traiettorie degli stati degli integratori
    for i in range(N):
        for m in range(M):
            plt.plot(time, x_trajectory[:, i, m])
    
    # Aggiungi le etichette per i valori di consenso
    for m in range(M):
